    ...
    <div style={{ background: 'rgba(27, 24, 27, 0.8)', padding: '40px', borderRadius: '15px', backdropFilter: 'blur(10px)', width: '380px', textAlign: 'center' }}>
      ...
      {step === 1 && (
        <form onSubmit={handleNext}>
          <input className="releaf-input" placeholder="ονοματεπώνυμο" value={form.full_name} onChange={e => setForm({...form, full_name: e.target.value})} />
          <input className="releaf-input" placeholder="username" value={form.username} onChange={e => setForm({...form, username: e.target.value})} />
          <input className="releaf-input" type="text" placeholder="email (π.χ. test@test.com)" value={form.email} onChange={e => setForm({...form, email: e.target.value})} />
          <input className="releaf-input" type="password" placeholder="password" value={form.password} onChange={e => setForm({...form, password: e.target.value})} />
      ...
      {step === 2 && (
        <form onSubmit={submit}>
          ...
          {/* Καθαρό Dropdown μόνο με Ελληνικά */}
          <select 
            className="releaf-input" 
            value={form.account_type} 
            onChange={e => setForm({...form, account_type: e.target.value})}
            style={{ WebkitAppearance: 'none', MozAppearance: 'none', appearance: 'none', textAlign: 'center', cursor: 'pointer' }}
          >
      ...
      {step === 3 && (
        <div style={{ padding: '20px 0' }}>
          <h2 style={{ fontFamily: 'var(--font-heading)', color: '#8db600', fontSize: '2rem', margin: '0 0 10px 0' }}>
            επιτυχία!
          </h2>
          <p style={{ fontFamily: 'var(--font-mono)', color: '#fff', lineHeight: '1.6' }}>
            ο λογαριασμός σας δημιουργήθηκε.<br/>
            καλώς ήρθατε στο releaf.
          </p>
          <button 
            className="releaf-button" 
            onClick={() => { setForm(initialFormState); setStep(1); onBack(); }} 
      ...
      {step !== 3 && (
        <>
          <br />
          <button className="releaf-button" type="button" style={{ background: 'transparent', border: '1px solid white', marginTop: '10px' }} onClick={() => { setForm(initialFormState); setErrorMessage(''); setStep(1); onBack(); }}>
            ακύρωση
          </button>
        </>
      )}
